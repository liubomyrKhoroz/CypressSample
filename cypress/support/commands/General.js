class General {
    ...
    locatorsGeneral = {
        ValidateErrorMessage: '.ant-col.ant-form-item-control >> .ant-form-item-explain-error',
        ValidateNotification: '.ant-message-custom-content > :nth-child(2)',
        OpenDevTools: '#6d7e865a-bc2f-4bd4-81a7-2925f674b192',
        OpenPageInput: '(//div[@class="ant-space ant-space-vertical"])[2]/div[3]//input',
    ...
    openPage(page) {
        ...
        cy.wait(100)
        cy.get(`[title="${page}"] > .ant-select-item-option-content`).as('btn').click({force: true})
        return this
    }
    ...
    verifyErrorMessageContain(order, message) {
        cy.get(this.locatorsGeneral.ValidateErrorMessage).eq(order).should('contain', message)
        return this
    }

    validateErrorMessage(order, message) {
        cy.get(this.locatorsGeneral.ValidateErrorMessage).eq(order).should('have.text', message)
        return this
    }

    validateNotification(message) {
        cy.get(this.locatorsGeneral.ValidateNotification).should('have.text', message)
        return this
    }
...
export function validateFieldAndPlaceholder(locator, placeholder) {
    cy.get(locator).should('not.have.value').and('have.attr', 'placeholder').and('eq', placeholder)
    return this
}
